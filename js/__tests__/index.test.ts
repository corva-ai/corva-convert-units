...
describe('lookup priority — shared unit keys resolve to the correct measure', () => {
  ...
  // --- gasConcentration: % must resolve to gasConcentration per v1.33.0 -----

  it('% resolves to gasConcentration (original library order)', () => {
    expect(describeUnit('%').measure).toBe('gasConcentration');
  });

  it('%→%EMA converts within gasConcentration', () => {
    expect(convert(1, '%', '%EMA')).toBeCloseTo(1, 6);
  });

  it('%→Fraction still works via pair lookup in proportion (50% = 0.5)', () => {
  ...
  // --- getUnitForPair: finds pairs inside non-default measures --------------

  it('converts bbl→Mscf via pair lookup in gasVolume', () => {
    // bbl globally resolves to volume, Mscf only exists in gasVolume;
    // the pair search must find gasVolume containing both.
    expect(convert(1000, 'bbl', 'Mscf')).toBeCloseTo((1000 * 0.0056) / 1, 4);
  });
  ...
  // --- exact unit key beats alias from another measure (v1.33.0 behavior) ---

  it('1/s resolves to angularVelocity (exact key beats strokesRate alias)', () => {
    expect(getUnit('1/s')?.measure).toBe('angularVelocity');
  });

  it('Hz alias resolves to 1/s in angularVelocity', () => {
    const unit = getUnit('Hz');
    expect(unit?.abbr).toBe('1/s');
    expect(unit?.measure).toBe('angularVelocity');
  });

  it('1/min alias still resolves to strokes/min in strokesRate', () => {
    const unit = getUnit('1/min');
    expect(unit?.abbr).toBe('strokes/min');
    expect(unit?.measure).toBe('strokesRate');
  });
});

describe('v1.33.0 resolution parity — every ambiguous unit key', () => {
  /**
   * Generated by running corva-convert-units@1.33.0 against every unit key
   * that exists in more than one measure. This table IS the reference
   * behavior — do not edit expectations without checking v1.33.0.
   */
  const V133_RESOLUTION: Record<string, string> = {
    '%': 'gasConcentration',
    J: 'energy',
    Nm: 'energy',
    Tbs: 'volume',
  ...
  for (const [abbr, measure] of Object.entries(V133_RESOLUTION)) {
    it(`${abbr} resolves to ${measure}`, () => {
      expect(getUnit(abbr)?.measure).toBe(measure);
    });
  }
});
