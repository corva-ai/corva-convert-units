...
class TestLookupPriority:
    ...
    # --- mass: g must resolve to mass (gram); old PY force had no g -----------

    def test_g_resolves_to_mass(self):
        assert describe("g")["measure"] == "mass"

    def test_g_to_kg(self):
        assert convert(1000, "g", "kg") == pytest.approx(1, abs=1e-9)

    # --- pressure_gradient: kPa/m and psi/ft belonged only to it in old PY ----

    def test_kPa_per_m_resolves_to_pressure_gradient(self):
        assert describe("kPa/m")["measure"] == "pressure_gradient"

    def test_psi_per_ft_resolves_to_pressure_gradient(self):
        assert describe("psi/ft")["measure"] == "pressure_gradient"
    ...
    # --- pair lookup: cross-measure pairs resolve within a single measure -----

    def test_bbl_to_Mscf_via_gas_volume_pair(self):
        # bbl globally resolves to volume, Mscf only exists in gas_volume;
        # convert() must find gas_volume containing both and use its anchors.
        assert convert(1000, "bbl", "Mscf") == pytest.approx(5.6, abs=1e-4)

    def test_percent_to_fraction_via_proportion_pair(self):
        # % globally resolves to gas_concentration, Fraction only in proportion.
        assert convert(50, "%", "Fraction") == pytest.approx(0.5, abs=1e-6)

    def test_truly_incompatible_measures_return_none(self):
        # No measure contains both ft and psi.
        assert convert(1, "ft", "psi") is None

    def test_percent_to_ema(self):
        assert convert(1, "%", "%EMA") == pytest.approx(1, abs=1e-6)

    def test_percent_to_ppm_gas_scale(self):
        assert convert(1, "%", "ppm") == pytest.approx(10000, abs=1e-4)

    # --- exact unit key beats alias from another measure ----------------------

    def test_one_per_s_resolves_to_angular_velocity(self):
        # 1/s is a canonical key in angular_velocity but also an alias of
        # strokes/sec in strokes_rate; the exact key must win.
        assert describe("1/s")["measure"] == "angular_velocity"

    def test_hz_alias_resolves_to_angular_velocity(self):
        unit = get_unit("Hz")
        assert unit["abbr"] == "1/s"
        assert unit["measure"] == "angular_velocity"

    def test_one_per_min_alias_resolves_to_strokes_rate(self):
        unit = get_unit("1/min")
        assert unit["abbr"] == "strokes/min"
        assert unit["measure"] == "strokes_rate"
...
# ---------------------------------------------------------------------------
# Old-repo resolution parity — every ambiguous unit key
# ---------------------------------------------------------------------------

# Generated by running the original corva-convert-units-py repo against every
# unit key that exists in more than one measure in the shared definitions.
# This table IS the reference behavior — do not edit expectations without
# checking the original repo.
#
# DELIBERATE DEVIATION: liquid-volume keys resolved to gas_volume in the old
# repo (the `volume` measure did not exist there), which produced incorrect
# results for liquid conversions (e.g. m3→bbl gave 630.36 instead of 628.98).
# They now resolve to `volume`. Gas-accounting anchors remain available via an
# explicit measure='gas_volume'.
_OLD_PY_RESOLUTION = {
    "%": "gas_concentration",
    "J": "energy",
    "Nm": "energy",
    ...
    "g": "mass",
    "kNm": "energy",
    "kPa/m": "pressure_gradient",
    "m": "length",
    "mV": "voltage",
    "ppm": "gas_concentration",
    "psi/ft": "pressure_gradient",
    "1/s": "angular_velocity",
    "1/min": "strokes_rate",
    "Hz": "angular_velocity",
}

_VOLUME_KEYS_DELIBERATE_FIX = [
    "Tbs", "bbl", "cl", "cm3", "cup", "dl", "fl-oz", "ft3", "gal", "glas",
    "in3", "kanna", "kkp", "km3", "krm", "l", "m3", "ml", "mm3", "msk",
    "pnt", "qt", "tsk", "tsp", "yd3",
]


class TestOldRepoResolutionParity:
    @pytest.mark.parametrize("abbr,measure", sorted(_OLD_PY_RESOLUTION.items()))
    def test_matches_old_repo(self, abbr: str, measure: str):
        unit = get_unit(abbr)
        assert unit is not None, f"{abbr} not found"
        assert unit["measure"] == measure

    @pytest.mark.parametrize("abbr", _VOLUME_KEYS_DELIBERATE_FIX)
    def test_volume_keys_use_liquid_anchors(self, abbr: str):
        unit = get_unit(abbr)
        assert unit is not None, f"{abbr} not found"
        assert unit["measure"] == "volume"
