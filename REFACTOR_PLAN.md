...
├── python/                            # PyPI package: corva-unit-converter
│   ├── pyproject.toml
│   ├── src/
...
- Move Python source from the sibling repo into `python/`
...
## Phase 3: Python package refactor (`python/`)
...
- JSON definitions bundled: `sync-definitions.sh` copies `definitions/*.json` into `python/src/corva_unit_converter/definitions/`

## Phase 4: Build and publish pipeline

### Scripts and Makefile

- **`scripts/sync-definitions.sh`** — copies `definitions/*.json` into `js/definitions/` and `python/src/corva_unit_converter/definitions/`. Both destination dirs are `.gitignore`d (derived artifacts).
- **Makefile targets:**
...
- **Definitions directory in packages is gitignored**: The copied JSON files in `js/definitions/` and `python/src/corva_unit_converter/definitions/` are derived artifacts, generated by the sync script. Only `definitions/` at the root is the source of truth.
