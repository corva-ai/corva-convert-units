...
describe('index', () => {
  ...
  describe('_resolveAbbrInMeasure', () => {
    const resolve = (abbr, measure) => convert(0)._resolveAbbrInMeasure(abbr, measure);

    it('returns abbr unchanged when measure does not exist', () => {
      expect(resolve('m', 'nonExistentMeasure')).toBe('m');
    });

    it('returns abbr when it is already a canonical unit key', () => {
      expect(resolve('psi', 'pressure')).toBe('psi');
      expect(resolve('ohmm', 'resistivity')).toBe('ohmm');
      expect(resolve('m', 'length')).toBe('m');
    });

    it('resolves alias to canonical key within resistivity', () => {
      expect(resolve('ohm.m', 'resistivity')).toBe('ohmm');
    });

    it('resolves PSI to psi within pressure', () => {
      expect(resolve('PSI', 'pressure')).toBe('psi');
    });

    it('resolves length aliases to m', () => {
      expect(resolve('meter', 'length')).toBe('m');
      expect(resolve('m.', 'length')).toBe('m');
    });

    it('resolves Hz to 1/s within angularVelocity', () => {
      expect(resolve('Hz', 'angularVelocity')).toBe('1/s');
    });

    it('returns abbr when no unit in measure matches key or alias', () => {
      expect(resolve('notARealUnit', 'pressure')).toBe('notARealUnit');
      expect(resolve('ohm.m', 'length')).toBe('ohm.m');
    });

    it('does not use units from another measure', () => {
      expect(resolve('kpsi', 'length')).toBe('kpsi');
    });
  });
});
