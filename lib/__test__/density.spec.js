  ...
  describe('density conversions stabilization', () => {
    for (let { from, to, amount, expected, precision = MEASUREMENT_PRECISION_AS_IS } of conversionTableStable) {
      it(`The conversion from ${from} to ${to} should be correct`, () => {
        expect(convert(amount).from(from, 'density').to(to, 'density')).toBeCloseTo(expected, precision);
      })
    }
  });
  ...
  describe('conversions', () => {
    for (let { from, to, amount, expected } of conversionTable) {
      it(`should correctly convert ${amount} "${from}" to "${to}"`, () => {
        expect(convert(amount).from(from, 'density').to(to, 'density')).toBeCloseTo(expected, MEASUREMENT_PRECISION);
      })
    }
  });
