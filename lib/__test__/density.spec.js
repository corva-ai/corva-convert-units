...
const conversionTableStable = [
  ...
  { from: 'lb/bbl', amount: 1, to: 'Sg', expected: 0.0028530095238095236 },
  { from: 'lb/bbl', amount: 1, to: 'kg/m3', expected: 2.8530095238095234 },
  { from: 'lb/bbl', amount: 1, to: 'kg/cm3', expected: 0.0000028530095238095238 },
  { from: 'lb/bbl', amount: 1, to: 'lb/gal', expected: 0.023809523809523808 },
  { from: 'lb/bbl', amount: 1, to: 'ppg', expected: 0.023809523809523808 },
  { from: 'lb/bbl', amount: 1, to: 'lb/Mgal', expected: 23.809523809523807 },
  { from: 'lb/bbl', amount: 1, to: 'psi/ft', expected: 0.0012368583797155224 },
  { from: 'lb/bbl', amount: 1, to: 'kPa/m', expected: 0.027978465846666665, precision: 8 },
  ...
  { from: 'psi/ft', amount: 1, to: 'ppg', expected: 19.25 },
  { from: 'ppg', amount: 12, to: 'psi/ft', expected: 0.6233766233766234 },

  { from: 'ppg', amount: 1, to: 'psi/1000ft', expected: 51.948, precision: 3 },
  { from: 'kg/m3', amount: 1000, to: 'lb/gal', expected: 8.3454, precision: 4 },

  { from: 'kg/m3', amount: 1000, to: 'kPa/m', expected: 9.80665, precision: 8 },
  { from: 'ppg', amount: 1, to: 'kPa/m', expected: 1.175095565559, precision: 8 },

  { from: 'mg/l', amount: 1, to: 'lb/Mgal', expected: 0.008345406354526215 },
  { from: 'ppg', amount: 1, to: 'mg/l', expected: 119826.40000000001 },
];
...
describe('density measurement', () => {
  ...
  describe('density conversions stabilization', () => {
    for (let { from, to, amount, expected, precision = MEASUREMENT_PRECISION_AS_IS } of conversionTableStable) {
      it(`The conversion from ${from} to ${to} should be correct`, () => {
        expect(convert(amount, 'density').from(from).to(to)).toBeCloseTo(expected, precision);
      })
    }
  });
  ...
  describe('conversions', () => {
    for (let { from, to, amount, expected } of conversionTable) {
      it(`should correctly convert ${amount} "${from}" to "${to}"`, () => {
        expect(convert(amount, 'density').from(from).to(to)).toBeCloseTo(expected, MEASUREMENT_PRECISION);
      })
    }
  });
