...
var convert, keys = require('lodash.keys'),
  ...
  measures = {
    ...
    surveyLength: require("./definitions/surveyLength"),
...
/**
 * Resolves alias or alternate spelling to canonical unit key within one measure.
 */
Converter.prototype._resolveAbbrInMeasure = function(abbr, measureName) {
  var systems = measures[measureName];

  if (!systems) {
    return abbr;
  }

  var systemNames = keys(systems);

  for (var si = 0; si < systemNames.length; si++) {
    var sys = systemNames[si];

    if (sys === '_anchors') {
      continue;
    }

    var units = systems[sys];
    var unitKeys = keys(units);

    for (var ui = 0; ui < unitKeys.length; ui++) {
      var unitKey = unitKeys[ui];
...
Converter.prototype.getUnit = function(abbr, measure) {
  ...
  if (measure) {
    abbr = this._resolveAbbrInMeasure(abbr, measure);
  ...
  var findUnitByCanonicalKey = function(canonicalKey) {
    var match = null;

    each(measures, function(systems, measureName) {
      each(systems, function(units, system) {
        if (system === '_anchors')
          return false;

        each(units, function(unit, testAbbr) {
          if (testAbbr === canonicalKey) {
            match = {
              abbr: canonicalKey,
              measure: measureName,
              system: system,
  ...
  // Prefer exact key match first (order of measures) so e.g. "1/s" stays angularVelocity, not strokesRate alias
  found = findUnitByCanonicalKey(abbr);

  if (!found) {
    var resolvedGlobal = this.getUnitKeyByAlias(abbr);

    if (resolvedGlobal !== undefined) {
      found = findUnitByCanonicalKey(resolvedGlobal);
    }
  }
