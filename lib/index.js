 ...
 * @returns {Converter}
 */
Converter.prototype.from = function(from, measure = undefined) {
  if (this.destination)
    throw new Error('.from must be called before .to');

  this.origin = this.getUnit(from, measure);
 ...
 */
Converter.prototype.to = function(to, measure = undefined) {
  if (!this.origin)
    throw new Error('.to must be called after .from');

  this.destination = this.getUnit(to, measure);
  ...
  if (this.destination.measure != this.origin.measure) {
    var pair = this.getUnitForPair(this.destination.abbr, this.origin.abbr, this.destination.measure, this.origin.measure);
    if (!pair) {
 ...
 * Finds the unit
 */
Converter.prototype.getUnit = function(abbr, measure = undefined) {
  var found;

  if (measure) {
    const isMeasureValid = this.measures().includes(measure);

    if (!isMeasureValid) {
      this.throwUnsupportedMeasureError(measure);
    }
...
Converter.prototype.getUnitForPair = function(abbrOne, abbrTwo, measureOne = undefined, measureTwo = undefined) {
  var foundOne;
  var foundTwo;

  if (measureOne || measureTwo) {
    const isMeasureOneValid = this.measures().includes(measureOne);
    const isMeasureTwoValid = this.measures().includes(measureTwo);

    if (!isMeasureOneValid) {
      this.throwUnsupportedMeasureError(measureOne);
    }

    if (!isMeasureTwoValid) {
      this.throwUnsupportedMeasureError(measureTwo);
    }

    if (measureOne) {
      const measure = measureOne;
      const systems = measures[measure];
      ...
      if (!foundOne) {
        this.throwUnsupportedCompatibilityError(abbrOne, measure);
      }
    }
    ...
    if (measureTwo) {
      ...
      if (!foundTwo) {
        this.throwUnsupportedCompatibilityError(abbrTwo, measure);
      }
    }
 ...
 */
Converter.prototype.describe = function(abbr, measure = undefined) {
  var resp = Converter.prototype.getUnit(abbr, measure);
